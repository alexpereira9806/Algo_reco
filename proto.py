...
def load_movies():
    # Get movie titles
    movies = {}
    with open("datasets/movielens/movies.csv") as src:
        reader = csv.DictReader(src)
        for line in reader:
            movies[line["movieId"]] = line["title"]


    # Load data
    prefs = defaultdict(dict)
    with open("datasets/movielens/ratings.csv") as src:
        reader = csv.DictReader(src)
        for line in reader:
            user = line["userId"]
            movie = movies[line["movieId"]]
            prefs[user][movie] = float(line["rating"])
